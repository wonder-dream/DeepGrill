...
| `visibility='pending'`（**保留值**，没有写入方 —— 见下） | 不出现在公共浏览里 |

⚠️ **`pending` 是一个没有写入方的保留值**：它在建表时按"晋升先入池、再等人审"的
形态留在 `CHECK` 里，而决策 5 / 68 之后的路是**先过闸再写库** ——
`promotion.run_gate()` 通过后直接写 `visibility='public'` + `primary_point_id=NULL`
（公共待定池，等知识层挂载）。所以下面那条排除**是防御性的，不是因为它会出现**。
澄清写在这里而不是改 `migrations/0001_initial.sql` 的注释：迁移文件校验和冻结
（决策 55），为一个字的注释制造一次假的"迁移漂移"告警不值得。
