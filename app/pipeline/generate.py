...
7. 绝对不要生成手撕算法题（白板手写代码题，如"手写快排""实现一个 LRU 缓存"）——本系统只收录口述问答/场景设计类面试题，面经中出现此类题目直接跳过不生成
8. 避免与已有题目重复：已有题目如下
{existing_stems}
9. tags 必须从以下词表中选择 1-5 个，不得使用词表外的词：
{vocab}
10. difficulty 按以下分级标准标注：
{difficulty_scale}
...
5. 绝对不要生成手撕算法题（白板手写代码题，如"手写快排""实现一个 LRU 缓存"）——本系统只收录口述问答/场景设计类面试题，出现此类需求直接跳过不生成
6. tags 必须从以下词表中选择 1-5 个，不得使用词表外的词：
{vocab}
7. difficulty 按以下分级标准标注：
{difficulty_scale}
