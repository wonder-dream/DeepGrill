...
def create_app(
    ...
    *,
    ...
    async def write_rate_limit_middleware(request: Request, call_next):
        ...
        write_ok, write_slots = _rl.reserve(key, WRITE_LIMIT)
        llm_slots = None
        if write_ok and is_llm_cost:
            write_ok, llm_slots = _rl.reserve(key, LLM_COST_LIMIT)
        if not write_ok:
            _rl.settle(key, write_slots, keep=False)  # LLM 额度不足被拒：通用额度也退回
            return JSONResponse(status_code=429, content={"detail": "操作过于频繁，请稍后再试"})
        response = await call_next(request)
        keep = response.status_code < 400
        _rl.settle(key, write_slots, keep=keep)
        _rl.settle(key, llm_slots, keep=keep)
        return response
