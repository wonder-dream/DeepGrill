...
个人求职准备工具：从授权/自选内容源（GitHub 面经仓库、手动导入、用户提交）生成面试题，向用户提问，答案由 LLM judge 按面试官标准判分并给出可行动的反馈；牛客源仅在本地个人学习场景可选启用。
...
| D2 | 数据源 | GitHub面经仓库 + 简历导入 + 手动导入；牛客(登录cookie)仅本地个人可选（默认关闭）；社交平台 MVP 仅预留接口（`social.py` 占位，采集延后） | 公开服务用授权/自选源与 UGC；牛客实时但授权边界不清，仅本地学习保留 |
...
| D13 | Cookie | 仅本地个人牛客源使用：手动从浏览器复制到本地 `.env`，gitignore 排除；公开/生产不配置 | 简单可靠；绝不入 git；避免进入公开流水线 |
...
```
┌─ 数据采集层 ────┐   ┌─ 生成层 ─────┐   ┌─ 交互层 ────┐   ┌─ 判分层 ────┐
│ GitHub仓库拉取   │   │ 清洗/规范化   │   │ 今日题目列表 │   │ 四维判分     │
│ 手动导入/用户提交│ → │ 归一化精确+embedding去重 │ → │ 答题页(深挖追问链)│ → │ 参考答案生成  │
...
```
定时触发(默认 08:00)
  ├─ 1. 采集: GitHub 仓库 git pull + 手动导入/用户提交（本地个人可加 牛客增量）
...
- `nowcoder.py`：**仅本地个人学习源（默认关闭，`sources.nowcoder_enabled=false`）**。带个人登录 cookie、随机间隔、重试退避；失败降级跳过（不影响其他源）。不进入公开/生产流水线。
...
nowcoder:
  cookie_env: NOWCODER_COOKIE     # 仅本地个人牛客源使用, gitignore
...
notification:
  enabled: true
sources:
  nowcoder_enabled: false         # 牛客采集开关：公开/生产默认关闭，仅本地个人学习显式开启
  github_repos: []                # 面经仓库列表
```
...
- 牛客仅限本地个人学习：默认 `sources.nowcoder_enabled=false`，公开/生产不注册；本地使用时随机间隔/重试退避/单源失败降级
- 本地服务只绑 127.0.0.1
...
| 风险 | 对策 |
|---|---|
| 牛客风控/改版导致本地源失效 | 牛客仅本地个人学习源，默认关闭；多源架构（GitHub/手动/UGC）+ 失败降级 + task_logs 可观察 |
