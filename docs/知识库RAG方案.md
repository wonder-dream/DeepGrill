...
- **相似度阈值** `KNOWLEDGE_MIN_SIM=0.5`：低相似块不注入（防不相关主题误导判分）
- **LLM 蒸馏** `--distill`：讲解/叙事型文档切块后 LLM 改写为高密度知识要点（删叙事/口语/类比，不添加新知识，失败降级保留原文）；蒸馏前删除同源旧块（重建语义）；实测 44→43 块、信息密度明显提升
- 经验：蒸馏改善信息密度，但不解决「知识库主题与题库不匹配」——后端题库需配套后端主题知识（Redis/MySQL/Java/Go 八股文）
